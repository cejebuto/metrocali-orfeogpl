<?php
/*
 *	Al cargar este codigo, si hay un municipio por defecto se cargan los 
 *   combos con la respectiva informacion, sino se cargan los combos vacios 
 *   y a traves de javascript vamos cambiando el contenido de los combos jerarquicamente.
 *	
 *   Creamos un recordset (y respectivo vector) para cada componente de direccion (Continentes,
 *   Paises, Dptos y Mnpios), usamos de "entrada" la opcion getmenu2 de ADODB para generar
 *   combos con las opciones por defecto.
 *	El vector es para crearlos en javascript y cambiar las opciones a medida que cambien
 *   lo seleccionado en los combos. 
 */

$ADODB_CACHE_DIR = session_save_path();
$ADODB_FETCH_MODE = ADODB_FETCH_ASSOC;
$db->conn->SetFetchMode(ADODB_FETCH_ASSOC);
//$db->conn->cacheSecs = 1800;
...
$sql_pais = "SELECT SGD_DEF_PAISES.ID_PAIS AS ID1,
                    SGD_DEF_PAISES.NOMBRE_PAIS AS NOMBRE,
                    SGD_DEF_PAISES.ID_CONT AS ID0
			FROM SGD_DEF_PAISES,
                    SGD_DEF_CONTINENTES
            WHERE SGD_DEF_PAISES.ID_CONT = SGD_DEF_CONTINENTES.ID_CONT
			GROUP BY SGD_DEF_PAISES.NOMBRE_PAIS,
                    SGD_DEF_PAISES.ID_CONT,
...
$sql_dpto = "SELECT $cad AS ID1,
            DEPARTAMENTO.DPTO_NOMB AS NOMBRE,
            DEPARTAMENTO.ID_PAIS AS ID0
			FROM DEPARTAMENTO, SGD_DEF_PAISES
			WHERE DEPARTAMENTO.ID_PAIS = SGD_DEF_PAISES.ID_PAIS
			GROUP BY $cad, DEPARTAMENTO.DPTO_NOMB, DEPARTAMENTO.ID_PAIS ORDER BY 2";
...
$sql_mcpo =	"SELECT $cad as ID1, MUNICIPIO.MUNI_NOMB as NOMBRE,
                    MUNICIPIO.DPTO_CODI as ID0,
                    MUNICIPIO.ID_PAIS as ID,
                    MUNICIPIO.HOMOLOGA_MUNI,
                    MUNICIPIO.HOMOLOGA_IDMUNI 
                FROM MUNICIPIO,
                    DEPARTAMENTO,
                    SGD_DEF_PAISES,
                    SGD_DEF_CONTINENTES 
                ...
                GROUP BY $cad, MUNICIPIO.MUNI_NOMB,
                    MUNICIPIO.DPTO_CODI,
                    MUNICIPIO.ID_PAIS,
                    MUNICIPIO.HOMOLOGA_MUNI,
                    MUNICIPIO.HOMOLOGA_IDMUNI 
