	...
	{  
	 case 'mssql':
	  ...
	  where 
		...
		and '.$db->conn->substr.'(convert(char(15),b.radi_nume_radi), 5, 3)=cast('.$dep_sel.' as varchar(5))
	...
	case 'postgres':
	 $isql = 'select
	  ...
	  where 
		b.radi_nume_radi is not null
		and b.radi_nume_radi=d.radi_nume_radi
		and '.$db->conn->substr.'(cast(b.radi_nume_radi as varchar(5)), 5, 3)=cast('.$dep_sel.' as varchar(5))
		and b.tdoc_codi=c.sgd_tpr_codigo
		'.$whereTpAnulacion.'
		'.$whereFiltro.'
	  order by '.$order .' ' .$orderTipo;
