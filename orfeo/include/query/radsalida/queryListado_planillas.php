...
switch($db->driver)
{
	...
	case 'postgres':
		...
		$query = "select SGD_RENV_CANTIDAD as CANTIDAD,
			'Certificado' 				as CERTIFICADO,
			".$db->conn->substr."(cast(RADI_NUME_SAL as varchar(18)),5,10) 	as REGISTRO,
			".$db->conn->substr."(SGD_RENV_NOMBRE,1,36) 			as DESTINATARIO,
			SGD_RENV_DESTINO 			as DESTINO,
			SGD_RENV_PESO			 	as PESO,
			'' 							as VALOR_PORTE,
			SGD_RENV_VALOR 				as CERTIFICADO,
			...
			".$db->conn->substr."(cast(RADI_NUME_GRUPO as varchar(18)),5,10) as RADI_NUME_GRUPO
			from SGD_RENV_REGENVIO ";
