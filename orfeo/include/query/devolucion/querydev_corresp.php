<?
	...
	break;
	case 'postgres':
		$where_depe = ' and cast('.$db->conn->substr.'(cast(radi_nume_salida as varchar(18)), 5, 3) as numeric) in ('.$lista_depcod .')';
		$sqlConcat = $db->conn->Concat("depe_codi","'-'","depe_nomb");
		$fecha_hoy = $db->conn->sysTimeStamp;
		$whereSubStrDepe = 'cast('.$db->conn->substr.'(cast(radi_nume_salida as varchar(18)), 5, 3) as numeric)';
		...
		$isql = 'select  
	        	a.radi_nume_salida as "Numero Radicacion"
			,a.anex_radi_nume  as "HID_anex_radi_nume"
			,'.$db->conn->substr.'(cast(radi_nume_salida as varchar(18)), 5, 3) as "Dependencia"
			,'.$fech_devol.' as "Fecha Devolucion"
			,'.$usua_devol.' as "Usuario Realiza Devolucion"
