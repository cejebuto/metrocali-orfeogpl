...
/*  Metro Cali S.A.                   2012-10               Eliminacion de consulta el campo depe_codi_aplica que no existe*/
/*************************************************************************************/
...
<?php
	...
	if($seriesVistaTodos!=1){
		$queryTip .= " and m.depe_codi = $dependencia ";
	}
			...
			if (confirm('Esta Seguro de Modificar el Registro de su Dependencia ?'))
				{
					...
					url="tipificar_documentos_transacciones.php?<?=session_name()."=".session_id()?>&modificar=1&usua=<?=$krd?>&codusua=<?=$codusua?>&tdoc=<?=$tdoc?>&tsub=<?=$tsub?>&codserie=<?=$codserie?>&coddepe=<?=$coddepe?>&codusuario=<?=$codusuario?>&dependencia=<?=$dependencia?>&nurad=<?=$nurad?>";
