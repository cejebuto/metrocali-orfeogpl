 ...
 * 
 * 
 * Modificación Metro Cali S.A. Oct -2012
 * Se trata de igualar las variables $idserie y $codserie que se usan en diferentes partes
 * 
 */
  ...
  <?php
	...
	$nomb_varde = "sgd_srd_descrip";
   	include "$ruta_raiz/include/query/trd/queryCodiDetalle.php";
        /* Modificación realizada por Metro Cali S.A.
         * Se cambio el campo id por sgd_srd_codigo para la siguiente consulta 
         */
	$querySerie = "select distinct ($sqlConcat) as detalle, sgd_srd_codigo 
	         from sgd_srd_seriesrd 
			 order by detalle 
			  ";
        /*Fin modificacion Metro Cali*/
		  ...
		  if($tsub !=0 && $codserie !=0 && $detasub !='')
		   {
			  $isqlB = "select * from sgd_sbrd_subserierd 
					  where sgd_srd_codigo = '$codserie'". //Modificado por Metro Cali S.A. se cambia sgd_srd_id por sgd_srd_codigo
			   ...
			   else 
			   {
	        	...
	        	if ($radiNumero !='') {
				   $mensaje_err = "<HR><center><B><FONT COLOR=RED>LA SERIE <$detasub > YA EXISTE. <BR>  VERIFIQUE LA INFORMACION E INTENTE DE NUEVO</FONT></B></center><HR>";
					...
					{
                                                //Modificado por Metro Cali S.A. se cambia SGD_SRD_ID por SGD_SRD_CODIGO
			...
			else
			{
			 echo "<script>alert('Los campos Serie, Subserie y Detalle son OBLIGATORIOS');</script>";
              ...
              //echo "<script>alert('Los campos". $detasub." ". $tsub." ". $codserie." son OBLIGATORIOS');</script>";
    }
   ...
   if($_POST['modi_subserie'] )
     {  
       if ($codserie !=0 && $tsub !=0 && $detasub != '')
        ...
        if($codserie && !is_null($codserie) && $codserie!=""){ //agregado por Metro Cali S.A.
            $idserie = $codserie;
